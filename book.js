const book = {
    bookArray: [
        ...
        {'id_book': "12", 'name': "Management", 'author': "Miller", 'isbn': "9780757524028"},
        {'id_book': "13", 'name': "Leadership Wisdom of Jesus", 'author': "Manz", 'isbn': "9781609940041"},
        {'id_book': "14", 'name': "Business Law Today", 'author': "Williams", 'isbn': "9780324786156"},
    ...
    getOneBook: function(x) {
        const foundBook = this.bookArray.find(book => book.id_book === String(x));
        if (foundBook) {
            console.log(foundBook);
        } else {
            console.log(`Book with id ${x} not found.`);
        }
    },

    // Lisää uuden kirjan bookArray-taulukkoon
    addBook: function(id_book, name, author, isbn) {
        const newBook = { id_book: String(id_book), name, author, isbn };
...
//book.getAllBooks(); 
//book.getOneBook(3); 
book.addBook(16, "Uusi Kirja", "Uusi kirjailija", "1234567890"); 
//book.getAllBooks();
